...
int setQueen(Stack *stack, int NumberofQueen) {
	...
	if (stack_is_empty(stack) == YES) {
		...
		ChessBoard *testBoard = (ChessBoard*)malloc(sizeof(ChessBoard));
		memset(testBoard, -1, sizeof(ChessBoard));

		memcpy(testBoard, stack->CurrentChessBoard, sizeof(ChessBoard));

		SaveNullBoard->NodeChessBoard = testBoard;

		Cor *testCor = (Cor*)malloc(sizeof(Cor));
		memset(testCor, -1, sizeof(Cor));
		//memset(SaveNullBoard->Corrdinate, -1, sizeof(Cor));
		//memcpy(SaveNullBoard->Corrdinate, testCor, sizeof(ChessBoard));

		SaveNullBoard->Corrdinate = testCor;
		SaveNullBoard->Corrdinate->x = -1;
		...
		// 
		// NewQueen->BeforeThisQueen->NodeChessBoard = testBoard;


		// *********** 문제 
		// 왜 체스보드판이 전부다 값이 바뀌는가? 
		// 왜 체스보드판이 싹 다 주소가 같은건지? 
		// 해결. -> memcpy
		...
		// Problem 
		// 3. 만약 이번 x행을 처음으로 조사하면, 현재 보드판을 가져오고, 이번 x번째 행에서 0인지점을 모두 차례대로 저장한다. 

		if (stack->QueenIndex = stack->Top->BeforeThisQueen->Corrdinate->x + 1) {
	...
	Recursive:
		...
		// 4. 만약 선택할 0이 없을 경우, 퀸이 놓아지지 않은 상태이므로 그냥 바로 전 노드로 돌아간다. **************여기 수정만 하면 될듯 
		
		if (stack->Top->AbleLocationArray_y[stack->Top->y_index] <0 || stack->Top->max_y_index == 0) {

			if (stack->QueenIndex == 0) {
			...
			stack->QueenIndex = stack->Top->Corrdinate->x + 1;

			if (stack->QueenIndex == 0) {
				return stack->AbleQueenCount;
			}

			// 5. 전 노드로 돌아갔는데, 퀸은 이미 놓여 있고, 하필 y인덱스값이 마지막이었다면? 퀸을 뺴야함 
			if (stack->Top->AbleLocationArray_y[stack->Top->y_index] < 0 && stack->Top->max_y_index == stack->Top->y_index) {

				// 그 전에있던 퀸 노드를 없애고 그 전으로 돌아가야함. 
				...
				memcpy(stack->CurrentChessBoard, stack->Top->NodeChessBoard, sizeof(ChessBoard));
				...
				memcpy(stack->CurrentChessBoard, stack->Top->BeforeThisQueen->NodeChessBoard, sizeof(ChessBoard));
			}
		}

		if( stack->Top->AbleLocationArray_y[stack->Top->y_index] >= 0 ){  // 6. 퀸들의 범위에서 벗어난 공간, 즉 0이 있을 경우 

			// 새 퀸의 x값은 그 전 퀸의 x값+1이다. 		
			stack->Top->Corrdinate = (Cor*)malloc(sizeof(Cor));
			stack->Top->Corrdinate->x = stack->Top->BeforeThisQueen->Corrdinate->x + 1;
			...
			// 이제 좌표 등록하고 
			stack->Top->Corrdinate->y = stack->Top->AbleLocationArray_y[stack->Top->y_index];

			stack->QueenAbleCoordinatesArray[stack->Top->Corrdinate->x][X] = stack->Top->Corrdinate->x;
			stack->QueenAbleCoordinatesArray[stack->Top->Corrdinate->x][Y] = stack->Top->Corrdinate->y;
			...
			stack->Top->BeforeThisQueen->NodeChessBoard = testBoard;

			/*if (stack->Top->Corrdinate->x == 5) {
				printf("test");
			...
			// 보드 등록하고 
			stack->Top->NodeChessBoard = stack->CurrentChessBoard;
			...
			if (stack->Top->Corrdinate->x == NumberofQueen - 1) {
