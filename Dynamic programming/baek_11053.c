...
int whichIsBig(int a, int b) {
	if (a > b) {
		...
		return SAME;
		// LIS 마지막 숫자와 지금 조사하는 정수의 값이 같다면 새로운 조사식을 도입해야 한다. 
	}
}
...
void fn(int N) {
	...
	while (idx != N) {
		...
		if (idx == 0) {
			...
			// 1. 일단, 입력된 전체 수열의 정수는 곧 어떤 LIS의 시작 부분이 될 수 있다. 따라서 일단 idx를 올릴 때, 그 정수를 배열에 넣어주자.
			arr[LENGTH][idx] = 1;
			arr[LATEST_NUM][idx] = num[idx];
			arr[CURRENT_NUM_INDEX][idx] = idx;

			// 2. 그리고, 지금까지 생성된 모든 가능한 경우의 LIS에 대하여, 현재 정수가 과연 그 LIS에 포함될 수 있는가에 대하여 조사해야 한다. 포함될 수 있는 경우와 아닌 경우로 나눈다. 만약 숫자가 같다면 논리가 추가된 로직을 만들어야한다. 
			...
			for(int search = 0; search < idx; search++){
				// 모든 LIS의 마지막 값과 현재 숫자값을 비교한다. 
				flag = whichIsBig(arr[LATEST_NUM][search], num[idx]);

				// 만약 LIS의 마지막 값이 현재 숫자값보다 크다면..
				if(flag == LEFT) {
				...
				// 만약 LIS의 마지막 값이 현재 숫자값보다 같다면..
				else if(flag == SAME){
					// 숫자값이 같아지면 문제가 조금 복잡해진다. 현재 LIS의 최댓값 숫자 위치부터 그 최댓값과 같은 현재 숫자의 위치 바로 전까지를 알아야 한다.
					...
					// 현재 LIS의 가장 큰 수의 위치를 저장
					startIdx = arr[CURRENT_NUM_INDEX][search];

					// 현재 num[idx] 의 위치를 저장
					endIdx = idx;

					// 그 LIS의 최댓값을 임시로 바로 전까지 최댓값이었던 값으로 바꿔준다. 
