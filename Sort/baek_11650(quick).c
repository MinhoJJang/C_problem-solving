...
void QuickSort_Y(cord arr[], int left, int right)
{
	if (left <= right)
	{
		int pivot = Partition_Y(arr, left, right); // pivot 정하고, 둘로쪼갬
		QuickSort_Y(arr, left, pivot - 1);		   // pivot 왼쪽영역을 퀵정렬
		QuickSort_Y(arr, pivot + 1, right);		   // pivot 오른쪽영역을퀵정렬
	}
}

void QuickSortMain_Y(cord arr[], int start, int end)
{
